...
PROGRAMS_DATA = [
    ...
    # ── 25. TYT BİYOLOJİ ──
    {
        ...
        "short_description": "derslerimizin tüm verimiyle başlamasıyla hazırlanan LGS ve YKS programları; canlı ders, deneme ve rehberlik desteğiyle tek platformda kolaylaşıyor. Sınav müfredatına uygun kaynaklarla konuşurken, profesyonel takip sistemiyle gelişiminizi anlık izleyebilirsiniz. Başarıya giden hazırlamak için eğitim programlarımızı hemen keşfedin.",
        "description": "Bu program sayesinden TYT Biyoloji sınavına hazırlık sürecinde varlıklar ve özelliklerinden ekolojiye kadar tüm müfredat konularını planlı ve sistemli bir şekilde çalışabilir, düzenli deneme sınavlarıyla seviyenizi anlık olarak takip edebilirsiniz. 4 haftalık eğitim programı kapsamında toplam 120 saat canlı eğitim gerçekleştirilir. Program süresince öğrenciler; kapsamlı konu anlatım videoları, geniş soru çözümü videoları ile biyolojinin temel prensiplerini ve görsel hafızaya dayalı konularını eksiksiz şekilde pekiştirir. Ayrıca süreç boyunca uygulanan 30 adet deneme ve seviye belirleme testi ile gelişim grafiği düzenli olarak ölçülür. Eğitim süresince akademik kariyer ve rehberlik danışmanlığı, birebir öğrenci koçluğu ve veli bilgilendirme sistemi eşlik eder. Program, bireysel öğrenme verimliliğini korumak amacıyla 20 kişilik sınırlı kontenjan ile yürütülür.",
        "curriculum_intro": "Bu program kapsamında TYT Biyoloji müfredatı öğrencinin hem ezberden uzak mantık kurmasını sağlayacak hem de görsel hafızasını destekleyecek bir yapıda ele alınır. Program süresince öğrenciler; 120 saatlik yoğun canlı ders maratonunda her konunun püf noktalarını öğrenirken, video içerikler ve zengin soru havuzuyla teorik bilgilerini pratiğe dönüştürürler. Uygulanan 30 adet deneme ve seviye belirleme testi, öğrencinin sınav provasını defalarca yapmasına olanak tanıyarak gerçek sınav stresini minimize eder. Akademik rehberlik, öğrenci koçluğu ve veli bilgilendirme sistemi sayesinde hazırlık süreci, öğrencinin motivasyonunu her zaman yüksek tutarak profesyonel bir takiple tamamlanır.",
        "kontenjan": 20,
        "start_date": "05.01.2026 20:00",
        "what_you_learn": [
        ...
        "faqs": [
            {"q": "dersler yerinde canlı mıdır?", "a": "Evet, eğitim programımızdaki tüm dersler alanında uzman eğitmenlerimizle etkileşimli ve canlı olarak işlenmektedir. Ders esnasında anlamadığınız yerleri anında sorabilirsiniz."},
            {"q": "Yapay Zeka Destekli Öğretim Sistemi Nasıl Çalışıyor?", "a": "Yapay zeka sistemimiz, katıldığınız deneme sınavlarındaki ve testlerdeki performansınızı analiz ederek eksik olduğunuz konuları belirler. Size özel ek çalışma yolları ve soru önerileri sunarak öğrenme verimliliğinizi maksimuma çıkarır."},
            {"q": "Öğretmenlerimizin deneyimi ve tecrübeleri nedir?", "a": "Platformumuzdaki tüm eğitmenlerimiz, TYT-AYT sınav gruplarında en az 5 yıl deneyime sahip, öğrenci psikolojisini ve sınav müfredatını çok iyi bilen profesyonellerden oluşmaktadır."},
            {"q": "Canlı dersler nasıl işleniyor?", "a": "Canlı derslerimiz, modern eğitim altyapımız üzerinden yüksek çözünürlüklü görüntü ve ses kalitesiyle işlenir. İnteraktif yazı tahtası ve anlık soru-cevap imkanı sunulur. Kaçırdığınız veya tekrar etmek istediğiniz derslerin kayıtlarını 7/24 izleyebilirsiniz."},
            {"q": "Deneme ve seviye belirleme testleri ne kadar sıklıkla oluyor?", "a": "Program boyunca toplam 30 adet deneme ve konu tarama testi uygulanır. Her ünitenin bitiminde seviye belirleme testleri ve düzenli aralıklarla genel TYT Biyoloji denemeleri yapılarak gelişiminiz yakından takip edilir."},
            {"q": "Rehberlik ve koçluk faaliyetleri nasıl işleniyor?", "a": "Birebir öğrenci koçluğu kapsamında haftalık gelişim takibiniz yapılır. Sınav stresi, zaman yönetimi ve ders çalışma programları bireysel olarak planlanır. Veli bilgilendirme sistemi ile de aileniz sürecinize dahil edilerek bilgilendirilir."},
            {"q": "Eğitim programlarına nasıl katılabilirim?", "a": "Sayfadaki 'Satın Al' butonuna tıklayarak kredi kartı (PayTR) veya Havale/EFT yöntemleriyle ödemenizi güvenle gerçekleştirebilirsiniz. Ödemenizin hemen ardından hesabınız aktif edilecek ve ders programınız tanımlanacaktır."}
        ],
        "reviews": [],
        "active": True
    },
...
def enrich_program_data(p_data):
    slug = p_data.get("slug")
    title = p_data.get("title", "")
    category = p_data.get("category", "")
    price = p_data.get("price", 3999900)
    hours = p_data.get("hours", 120)
    
    orig_price = int(price * 1.25)
    ...
    # Merge back into p_data
    p_data["original_price"] = orig_price
    p_data["subtitle"] = "okuldaveevde"
    p_data["short_description"] = short_description
    p_data["description"] = description
    p_data["curriculum_intro"] = curriculum_intro
    p_data["kontenjan"] = 20
    p_data["start_date"] = start_date
    
    if not p_data.get("what_you_learn") or len(p_data.get("what_you_learn")) <= 3:
        p_data["what_you_learn"] = what_you_learn
    if not p_data.get("faqs"):
        p_data["faqs"] = faqs
    if not p_data.get("reviews"):
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            for p_data in PROGRAMS_DATA:
                enriched = enrich_program_data(p_data)
                program = EducationProgram(**enriched)
                session.add(program)
