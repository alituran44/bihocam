...
export default function TeacherProfilePage() {
  ...
  const handleBuyPackage = () => {
    if (selectedSlotId) {
      router.push(`/checkout?type=package&teacher_id=${teacherId}&weeks=${selectedWeeks}&hours=${selectedHours}&slot_id=${selectedSlotId}&format=${lessonFormat}`);
    } else {
      setActiveTab("book");
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div id="live-class-section" className="bg-white rounded-[2rem] border border-slate-100 p-6 sm:p-8 shadow-sm space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4 border-b border-slate-100/50">
                    {[
                      ...
                      { 
                        label: "Online Ders", 
                        value: data.teacher.live_class_price ? `${data.teacher.live_class_price} ₺/saat` : "Anlaşmalı", 
                        icon: "💻" 
                      },
                      { 
                        label: "Yüz Yüze Ders", 
                        value: data.teacher.face_to_face_price ? `${data.teacher.face_to_face_price} ₺/saat` : (data.teacher.face_to_face_price === 0 ? "Ücretsiz" : "Belirtilmedi"), 
                        icon: "📍" 
                      },
                      { label: "Grup Dersi Ücreti", value: "İndirimli / Koltuk", icon: "👥" },
                    ].map((item, idx) => (
                          ...
                          <div className="space-y-1.5 pb-2 border-b border-slate-100/60">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-[10px] font-bold text-slate-400">
                                {lessonFormat === "online" ? "Google Meet / Canlı" : "Yüz Yüze Görüşme"}
                              </span>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              >
                                ...
                                <div className="text-[11px] font-black text-blue-600">
                                  {data.teacher.live_class_price ? `${data.teacher.live_class_price} ₺/saat` : "Anlaşmalı"}
                                </div>
                              </button>
                              ...
                              >
                                ...
                                <div className="text-[11px] font-black text-indigo-600">
                                  {data.teacher.face_to_face_price ? `${data.teacher.face_to_face_price} ₺/saat` : (data.teacher.face_to_face_price === 0 ? "Ücretsiz" : "Belirtilmedi")}
                                </div>
                              </button>
                            </div>
                          </div>
            ...
            <div className="lg:col-span-4 space-y-6 sticky top-24">
              ...
              <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 space-y-5">
                ...
                {(lessonFormat === "online" ? data.teacher.live_class_price : (data.teacher.face_to_face_price || data.teacher.live_class_price)) && (
                  <div className="space-y-4">
                    {/* Hour slider input */}
                    <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {WEEK_OPTIONS.map((opt) => {
                        ...
                        const baseHourly = lessonFormat === "online"
                          ? (data.teacher.live_class_price || 0)
                          : (data.teacher.face_to_face_price || data.teacher.live_class_price || 0);
                        const packageCost = calcPackage(baseHourly, selectedHours, opt.weeks);
