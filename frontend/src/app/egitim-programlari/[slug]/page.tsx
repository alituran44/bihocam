        ...
        <section className="max-w-7xl mx-auto px-4 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              {/* Video Player */}
              <div className="rounded-2xl overflow-hidden shadow-xl bg-gray-900 aspect-video relative group cursor-pointer" onClick={() => setVideoPlaying(!videoPlaying)}>
                <div className={`w-full h-full bg-gradient-to-br ${prog.gradient || "from-gray-900 via-gray-800 to-gray-700"} flex items-center justify-center`}>
                  <div className="text-center">
                    ...
                    <p className="text-white/60 text-sm font-semibold">{videoPlaying ? "▶ Oynatılıyor..." : "Tanıtım videosunu izle"}</p>
                  </div>
              ...
              {/* Title */}
              <div>
                <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-3 leading-tight">{prog.title}</h1>
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="flex items-center gap-2">
                    <Stars rating={prog.rating} size="md" />
                    <span className="font-bold text-yellow-600">{prog.rating}</span>
                    <span className="text-gray-500 text-sm">({(prog.review_count || 1200).toLocaleString("tr-TR")} değerlendirme)</span>
                  </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                <p className="text-gray-600 leading-relaxed font-medium text-sm whitespace-pre-wrap">{prog.description}</p>
              </div>
              ...
              {prog.what_you_learn && prog.what_you_learn.length > 0 && (
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl border border-teal-100 p-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-5">Bu eğitimde neler öğreneceksiniz?</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {prog.what_you_learn.map((item: string, i: number) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <p className="text-sm text-gray-700 leading-relaxed font-semibold">{item}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {prog.curriculum && prog.curriculum.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-2">Eğitim İçeriği</h2>
                  <p className="text-sm text-gray-500 mb-5 font-semibold">
                    {prog.curriculum.reduce((a: number, s: any) => a + (s.lessonCount || s.items?.length || 0), 0)} ders · {prog.hours} saat toplam süre
                  </p>
                      ...
                      <div key={i} className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-extrabold text-gray-900 text-sm">{section.title}</span>
                          </div>
                          <span className="text-xs text-gray-400 font-bold flex-shrink-0">{section.lessonCount || section.items?.length || 0} ders · {section.duration}</span>
                        </button>
                  ...
                  <div className="space-y-3">
                    {prog.faqs.map((faq: any, i: number) => (
                      <div key={i} className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
                        ...
                        >
                          <span className="font-extrabold text-gray-900 text-sm pr-4">{faq.q}</span>
                        ...
                        {openFaq === i && (
                          <div className="px-5 pb-4 text-xs sm:text-sm text-gray-600 leading-relaxed font-semibold bg-teal-50/20 border-t border-teal-50/50 pt-3">
                            {faq.a}
                          </div>
                        )}
                      </div>
              ...
              {prog.reviews && prog.reviews.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <div className="text-center flex-shrink-0 bg-gray-50 p-4 rounded-2xl border border-gray-100 shadow-inner">
                      <p className="text-4xl font-black text-gray-900">{prog.rating}</p>
                      <Stars rating={prog.rating} size="sm" />
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-1">Eğitim Puanı</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {prog.reviews.map((rev: any, i: number) => (
                      <div key={i} className="bg-gray-50 border border-gray-200/50 rounded-2xl p-5 shadow-sm">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24">
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                  {/* Program visual */}
                  <div className={`h-36 bg-gradient-to-br ${prog.gradient || "from-gray-900 via-gray-800 to-gray-700"} flex items-center justify-center shadow-inner`}>
                    <div className="text-center px-4">
                      <p className="text-white/60 text-xs font-semibold uppercase tracking-widest">{prog.category}</p>
                      <p className="text-white font-black text-lg leading-tight mt-1">EĞİTİM PROGRAMI</p>
                    </div>
                  </div>
                  ...
                  <div className="p-5 space-y-4">
                    {/* Price */}
                    <div className="text-center">
                      <p className="text-3xl font-black text-gray-900">{formatPrice(prog.price)} TL</p>
                      {prog.original_price && (
                        <p className="text-gray-400 line-through text-sm font-semibold">{formatPrice(prog.original_price)} TL</p>
                      )}
                      <p className="text-[10px] text-gray-400 font-bold mt-0.5">+ KDV</p>
                    </div>

                    {/* Rating */}
                    <div className="flex items-center justify-center gap-2 bg-gray-50 rounded-xl py-2 border border-gray-100 shadow-inner">
                      <Stars rating={prog.rating} size="sm" />
                      <span className="text-xs font-bold text-gray-700">{prog.rating} ({(prog.review_count || 1200).toLocaleString("tr-TR")} yorum)</span>
                    </div>

                    {/* Buttons */}
                    <Link
                      href={`/checkout?program=${prog.slug}`}
                      className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-bold rounded-xl hover:shadow-xl hover:shadow-teal-500/30 hover:-translate-y-0.5 transition-all text-base cursor-pointer shadow-md"
                    >
                    ...
                    <Link
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3 border-2 border-gray-200 text-gray-700 font-bold rounded-xl hover:border-teal-300 hover:text-teal-700 transition-all text-sm cursor-pointer"
                    >
                      Tüm Programları Gör
                    </Link>
                    ...
                    <div className="border-t border-gray-100 pt-4 space-y-3">
                      {[
                        { icon: "🎓", text: "Ömür boyu erişim" },
                        { icon: "📱", text: "Mobil ve masaüstü" },
                        { icon: "🏆", text: "Sertifika" },
                        { icon: "💯", text: "30 gün para iadesi" },
                        { icon: "🤖", text: "AI destekli öğrenme" },
                      ].map((f) => (
                        <div key={f.text} className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 font-semibold">
                          <span>{f.icon}</span>
                          <span>{f.text}</span>
                        </div>
                      ))}
